...
We have to identify the logical elements that make up our App. This is actually simpler for engineering applications, as we are dealing with physical objects. In an abstract logical problem, you can easily get lost trying to figure out what constitutes an entity,

I want to emphasize one important way to thinking that is essential for proper object-oriented programming.

Traditionally you would look at the function that needs to be performed on some data, and treat the data as a passive entity. In a way, you think "I need to move the car to this position, so I will write a function to move the car from A to B, and pass it the car as a parameter." The car in this way of thinking, is just a passive thing, and all the work is done to it "from the outside".

In object-oriented programming, you keep the functionality close to the object. So the above function would change to "Car, move yourself to this position." Sometimes this way of thinking may seem counter-intuitive. Isn't the driver the one, who drives the car? Shouldn't there be a `Driver` object that interacts with the car in some standardized way? Absolutely, this is a very valid question. And making these choices right is key to good software architecture. A good software architect will always try to minimize the information that needs to be exchanged in order to arrive at a goal. Information hiding and limiting exposed functionality to essentials is key to robust code.

Let's look at a powerful concept in our toolbox, which can help us on our quest to simplify our interactions with objects.

## Common ancestors

If you think about the world, the things that surround us have a lot in common. Almost everything is an object of some sort. And these objects have many things in common. We expect, for example, all physical objects to occupy a position in space, have a weight, etc.

So, in a way, it makes sense to group objects almost like in a family tree — a bit like what we find in biology. There are plants, there are animals, some animals are mammals, whereas others are fish, etc.

In object-oriented programming, we use *Inheritance* to build up a hiearchical tree of related objects. We can implement certain common functionality and properties in a *Base Class*. That functionality is then available in all classes that derive from it. Let's go back to our car example.

Some of the things, like the name of our car, are not unique to the `TeslaRoadster` class. They could be moved to a `Car`class. And basically everyhing, maybe with the exception of the implementation of a specific battery degradation formula, could be moved to an `ElectricCar` class that is derived from the more general `Car` class. This would avoid a lot of repetition of code, and make it incredibly easy to create new classes, as all we would have to do is implement the things that are specific to that class.
...
In our constructor we have to make sure we call the constructor of the base class. We do this by adding `: base(...)` at the end of the constructor, as below. This passes the name of the parameter to the base class constructor. If we omit this call, it still works, but the base constructor will use the default value for the constructor parameters.
...
So, now we have a new way to think about our `TeslaRoadster` object. Yes, it's a `TeslaRoadster`, but it is also a `Car`. And when I don't need to deal with anything specific to this specific type of cars, I can just work with `Car` variables.
...
Here we have added three cars to a list (a storage container). Even though we clearly created a certain type of car, we have added them all to a generic list, that holds objects of type `Car`. We have then used a `foreach` loop to list all the cars' names.

For the purpose of printing out the names, we really don't car what specific class of `Car` the list actually holds at this position.

## Polymorphism

I always hated that word. It makes something very simple sound complicated.

Let's imagine we want to provide a common way expose functionality for all objects of a certain base type. But the actual *implementation* may differ. We could have a function `PrepareForDriving`, which in a gasoline car would call the `Refuel` function, but in an electric car, it would call `Charge`. How can we get there?
...
While we are dealing with a variable `oCar` of type `Car`, the actual object stored in it is actually a `TeslaRoadster`, which implements a different version of `PrepareForDriving` that our base class `Car`. Because `PrepareForDriving` is a `virtual` function, the function of the actual type, which is `TeslaRoadster.PrepareForDriving()`, is called, and not the one in the base type.
...
We can force the programmer's hand by adding the keyword `abstract` to the base class implementation. When we do this, the derived class *must* implement the function, otherwise the compiler will throw an error. We must also mark the entire class as `abstract` to indicate that it's now actually impossible to instantiate a `Car`. Because `Car` doesn't implement `PrepareForDriving`, it has become an abstract class, that can only serve as the base for derived classes and can no longer stand on its own. This makes sense, since it's more of a category than an actual object. Just like you cannot have a "Fish", it needs to be a "Blue Fin Tuna", which is a proper fish in reality.
